...
#!/usr/bin/env python3
"""
Simple Agent With File - A simple example AI agent.
...
class SimpleAgentWithFileHandler(BaseHTTPRequestHandler):
    """HTTP handler for the Simple Agent With File."""

    def do_GET(self):
        """Handle GET requests."""
        if self.path == '/health':
            self._send_json_response({"status": "healthy", "agent": "simple-agent-with-file"})
        elif self.path == '/':
            self._send_json_response({
                "message": "Hello from Simple Agent With File!",
                "endpoints": [
    ...
    async def _handle_runcmd_async(self, prompt: str, thread_id: str, turn_id: str | None = None, files: list[dict] | None = None):
        ...
        try:
                ...
                for node_name, node_output in chunk.items():
                    ...
                    for last_message in messages:
                        ...
                        if msg_type == "AIMessage":
                            ...
                            # Check for custom payload (chart/table)
                            try:
                                json_content = json.loads(content)
                                if isinstance(json_content, dict) and json_content.get("__jr__"):
                                    await client.append(Event(
                                        thread_id=thread_id,
                                        turn_id=turn_id,
                                        type="custom",
            ...
            await client.append(Event(
                ...
                level=Level.INFO,
                payload={"output": "", "message_count": message_count},
            ))
    ...
    def _handle_runcmd(self):
        ...
        try:
            ...
            def run_async_in_thread():
                asyncio.run(self._handle_runcmd_async(prompt, thread_id, turn_id, files))
...
def main():
    """Main function to run the Simple Agent With File."""
    port = int(os.environ.get('PORT', 8080))

    logger.info(f"Starting Simple Agent With File on port {port}")
    logger.info(f"Health check: http://0.0.0.0:{port}/health")
    logger.info(f"Runcmd endpoint: http://0.0.0.0:{port}/runcmd")
    logger.info(f"Agent endpoint: http://0.0.0.0:{port}/")

    server_address = ('', port)
    httpd = HTTPServer(server_address, SimpleAgentWithFileHandler)

    try:
        logger.info(f"Simple Agent With File is running on port {port}")
        httpd.serve_forever()
    except KeyboardInterrupt:
        logger.info("Shutting down Simple Agent With File")
        httpd.server_close()
