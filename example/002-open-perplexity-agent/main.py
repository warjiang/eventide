...
class SimpleAgentHandler(BaseHTTPRequestHandler):
    """HTTP handler for the Open Perplexity Agent."""

    def do_GET(self):
        """Handle GET requests."""
        if self.path == '/health':
            self._send_json_response({"status": "healthy", "agent": "open-perplexity-agent"})
        elif self.path == '/':
...
def main():
    """Main function to run the Open Perplexity Agent."""
    ...
    try:
        logger.info(f"Open Perplexity Agent is running on port {port}")
        httpd.serve_forever()
    except KeyboardInterrupt:
        logger.info("Shutting down Open Perplexity Agent")
        httpd.server_close()
